...
func (wrapper *SimpleSettingWrapper) Get(key string) (string, error) {
	...
	if !result.Success() {
		if errs := result.Errors(); len(errs) == 0 {
			return value, errors.New("Operation get failed to execute in Setting Wrapper")
		} else {
			return value, errs[0]
		}
	}
...
func (wrapper *SimpleSettingWrapper) Set(key, value string) error {
	...
	if !result.Success() {
		if errs := result.Errors(); len(errs) == 0 {
			return errors.New("Operation get failed to execute in Setting Wrapper")
		} else {
			return errs[0]
		}
	}
...
func (wrapper *SimpleSettingWrapper) List(parent string) ([]string, error) {
	...
	if !result.Success() {
		if errs := result.Errors(); len(errs) == 0 {
			return list, errors.New("Operation get failed to execute in Setting Wrapper")
		} else {
			return list, errs[0]
		}
	}
